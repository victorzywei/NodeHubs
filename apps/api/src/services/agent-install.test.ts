...
describe('agent install scripts', () => {
  ...
  it('builds a generic installer with network, runtime, and warp steps', () => {
    ...
    expect(script).toContain('tag="$(get_latest_github_tag "XTLS/Xray-core" "v26.2.6")"')
    expect(script).toContain('[ -n "$tag" ] || tag="v26.2.6"')
    expect(script).toContain('tag="$(get_latest_github_tag "SagerNet/sing-box" "v1.13.2")"')
    expect(script).toContain('[ -n "$tag" ] || tag="v1.13.2"')
    ...
    expect(script).toContain('<key>service_mode</key>')
    expect(script).toContain('<key>proxy_port</key>')
