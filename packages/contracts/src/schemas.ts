...
export const updateNodeSchema = nodeSchemaBase.partial().extend({
  ...
  warpIpv4: z.string().trim().max(255).optional(),
  warpIpv6: z.string().trim().max(255).optional(),
  warpEndpoint: z.string().trim().max(255).optional(),
  warpAccountType: z.string().trim().max(120).optional(),
  warpTunnelProtocol: z.string().trim().max(120).optional(),
  warpPrivateKey: z.string().trim().max(255).optional(),
  warpReserved: z.array(z.number().int().min(0).max(255)).length(3).nullable().optional(),
  argoStatus: z.string().trim().max(120).optional(),
  argoDomain: z.string().trim().max(255).optional(),
  permissionMode: z.enum(['root', 'user']).optional(),
  singBoxVersion: z.string().trim().max(255).optional(),
  singBoxStatus: z.string().trim().max(120).optional(),
  xrayVersion: z.string().trim().max(255).optional(),
  xrayStatus: z.string().trim().max(120).optional(),
